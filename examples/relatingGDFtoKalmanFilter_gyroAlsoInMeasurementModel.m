...
includeGyroTransformationInMeasurementModel = false;
%% Sensor Variance
syms dt
...
A = sym.eye(4); % State transition model

q_p = A*q;  % predicted state
P_p = A*P*A.'; %PRedicted state covariance
...
if includeGyroTransformationInMeasurementModel
    syms wx wy wz
    wvars = [wx;wy;wz];
    ...
    omega4=[0,-wx,-wy,-wz;
        ...
        wy,-wz,0,wx;
        wz,wy,-wx,0];
    H = [eye(4) + dt/2*omega4;eye(4);eye(4);];
else
    wvars = sym.empty;
    H = [eye(4);eye(4);eye(4)]; % Observation Model
end
...
repVars = [pVars qpVars QVars RVars WVars];
repVars(repVars == dt) = [];
