...
% Ryan Riddick edit:
% Also flip the sign of the angle (this is needed for when quaternions are
% converted to euler angles and there's a phase jump
DD = diff(dp_corr);
if any(DD~=0)
    x=1;
end
...
% A certain range in which euler angles are flipped
dd = 109:142;
% Fix 
pcF = -PC(dd);
pcF = pcF - pcF(end) + PC(dd(end)+1);
PC(dd) = pcF;
figure;plot(PC)

jumpDex = find(dp_corr)+1;
pFlip = p;

strategy = 'assumeStartIsCorrect';
for i = 1 : length(jumpDex)
    switch strategy
        case 'assumeStartIsCorrect'
            x0 = p(jumpDex(i)-1);
            startDex = jumpDex(i);
            if i < length(jumpDex)
                endDex = jumpDex(i+1);
        ...
        case 'assumeStartIsIncorrect'
            if i > 1
                startDex = jumpDex(i-1);
            else
                startDex = 1;
            end
            endDex = jumpDex(i)-1;
    ...
    flipDex = startDex:endDex;
    pFlip(flipDex) = x0 - (p(flipDex)-p(startDex));
end