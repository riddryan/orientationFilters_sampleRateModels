function [qPredicted,qCorrected,pPredicted,R,J,L,E] = rEKF_update(q,gyro,acc,mag,Sigma_g,Sigma_a,Sigma_m,dt,P)
%% Set up
accMag = norm(acc);
magMag = norm(mag);
acc = acc / accMag;
...
% ra11 = Sigma_a(1,1);
% Sigma_a = Sigma_a + diag(3)*(1-yA) * 10 * ra11;

muM = 55.277;
sigM = 15;
yM = exp(-(magMag - muM).^2 / (2*sigM^2));
rm11 = Sigma_m(1,1);
Sigma_m = Sigma_m + diag(3)*(1-yM) * 1 * rm11;
