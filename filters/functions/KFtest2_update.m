function [qPredicted,qCorrected,pPredicted,R,J,L,E] = KFtest2_update(q,gyro,acc,mag,Sigma_g,Sigma_a,Sigma_m,dt,P)
%% Normalize
acc = acc / norm(acc);
mag = mag / norm(mag);
R = NaN(4,4);
...
[qMeasured,J] = processMeasurement(q,acc,mag);
R = measurementCovariance(Sigma_a,Sigma_m,J);
...
[qEstimated,PEstimated,L] = kalmanUpdate(qPredicted,qMeasured,A,Q,R,P);
...
function [qEstimated,PEstimated,L] = kalmanUpdate(qPredicted,qMeasured,A,Q,R,P)
