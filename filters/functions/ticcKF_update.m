function [qPredicted,qCorrected] = ticcKF_update(q,gyro,acc,mag,gains,dt)
can2 = gains(1);
can0 = gains(2);
cmn2 = gains(3);
cmn0 = gains(4);
...
% CA = (can2*dt^2 + can1*dt + can0) / (dt^2 + cad1*dt + cad0);
% CM = (cmn2*dt^2 + cmn1*dt + cmn0) / (dt^2 + cmd1*dt + cmd0);
% CA = (can2*dt^2) / (d2*dt^2 + d0);
% CM = (cmn2*dt^2) / (d2*dt^2 + d0);

            CA = (can2*dt^2 + can0) / (dt^2 + d0);
            CM = (cmn2*dt^2 + cmn0) / (dt^2 + d0);

%             qCorrected = (q - obj.c_accelerometer * qa - obj.c_magnetometer * qm);
qCorrected = (q - CA * dqAcc.' - CM * dqMag.');
