function [qPredicted,qCorrected,pPredicted,R,J,L,E] = EKFtestGref_update(q,gyro,acc,mag,Sigma_g,Sigma_a,Sigma_m,dt,P)
%% Set up
acc = acc / norm(acc);
mag = mag / norm(mag);
...
%% Measurement
expAcc = [-2*q(1)*q(3) + 2*q(2)*q(4)
    2*q(1)*q(2)   + 2*q(3)*q(4)
    q(1)*q(1)     - q(2)*q(2) - q(3)*q(3) + q(4)*q(4)];
Fa = expAcc - [acc(1);acc(2);acc(3)];
...
% Jm = [2*q4 + 2*my*q1 - 2*mz*q2, 2*q1 - 2*mx*q1 - 2*mz*q3, 2*mx*q2 - 2*q2 + 2*my*q3
% 2*q3 - 2*my*q2 - 2*mz*q1, 2*mx*q2 - 2*q2 + 2*mz*q4, 2*mx*q1 - 2*q1 - 2*my*q4
% 2*q2 - 2*my*q3 - 2*mz*q4, 2*q3 + 2*mx*q3 - 2*mz*q1, 2*q4 + 2*mx*q4 + 2*my*q1
% 2*q1 + 2*my*q4 - 2*mz*q3, 2*mz*q2 - 2*mx*q4 - 2*q4, 2*q3 + 2*mx*q3 - 2*my*q2];
