function [qPredicted,qCorrected,pPredicted,R,J,L,E] = aEKF_update(q,gyro,acc,mag,Sigma_g,Sigma_a,Sigma_m,dt,P,accMean)
%% Set up
accMag = norm(acc);
magMag = norm(mag);
acc = acc / accMag;
...
muA = 1;
sigA = 0.2;
yA = exp(-(accMean - muA).^2 / (2*sigA^2));
% yA = exp(-(accMag- muA).^2 / (2*sigA^2));
ra11 = Sigma_a(1,1);
Sigma_a = Sigma_a + eye(3)*(1-yA) * 4 * ra11;

% muM = 55.277;
% sigM = 15;
% yM = exp(-(magMag - muM).^2 / (2*sigM^2));
% rm11 = Sigma_m(1,1);
% Sigma_m = Sigma_m + eye(3)*(1-yM) * 1 * rm11;
